...
def pageBadRequest(request, exception):
    exc = {'codeNumber': '400', 'errorText': 'Ошибка', 'descText': 'Ошибка отправленного запроса.'}
...
def pageForbidden(request, exception):
    exc = {'codeNumber': '403', 'errorText': 'Ошибка', 'descText': 'Доступ запрещён!'}
    return render(request, 'PicMeMain/exception.html', context=exc)
    # return HttpResponseForbidden('<h1>Ошибка 403 - Нету доступа!</h1>')


def pageNotFound(request, exception):
    exc = {'codeNumber': '404', 'errorText': 'Ошибка', 'descText': 'Данная страница не была найдена.'}
    return render(request, 'PicMeMain/exception.html', context=exc)
    # return HttpResponseNotFound('<h1>Ошибка 404 - Не найдено!</h1>')


def pageServerError(exception):
    exc = {'codeNumber': '500', 'errorText': 'Ошибка', 'descText':'Ошибка на стороне сервера.'}
